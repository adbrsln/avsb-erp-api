...
- **Command** `attendance:auto-clock-out` (`app/Console/Commands/AutoClockOut.php`) — options `--dry-run` (preview, zero writes), `--grace=` (override). Self-gated: `CompanySetting::value('auto_clock_out_enabled')` false → info + SUCCESS. Scheduled in `routes/console.php`: **everyThirtyMinutes()** (was everyFiveMinutes — cadence does NOT affect close precision: `closeAt = work_end + grace` is deterministic; cadence only sets detection latency. 30min = notification ≤30min late, zero payroll impact). Self-gating makes always-scheduled safe.
- **MySQL `time` column gotcha (prod crash)**: migration `2026_07_31_000200` uses `$table->time()` — MySQL returns `17:00:00`, UI sends `17:00`, SQLite (tests) stores raw. `Carbon::createFromFormat('H:i', $end)` on `17:00:00` → **"Trailing data"** (Creator.php:625) and the whole command crashes. FIXED in `resolveCloseAt()`: trim → `substr(0, 5)` → regex guard `^\d{1,2}:\d{2}$` (invalid → skip that session, no crash). Regression test: `handles MySQL time column format (H:i:s)`. Note `scheduleFlagReason()` never hit this — uses tolerant `strtotime()`.
