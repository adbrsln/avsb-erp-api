...
use App\Services\Payroll\PayrollStatutoryService;
...
class PayrollController extends Controller
{
    ...
    public function reverseItem(Request $request, int $id, int $itemId): JsonResponse
    {
        $item = PayrollRunItem::find($itemId);
        if (! $item || $item->period_id !== $id) {
            return response()->json(['error' => 'Payroll item not found'], 404);
        }

        $user = $request->user();
        $staff = $user ? StaffProfile::where('email', $user->email)->first() : null;

        $result = (new PayrollReversalService)->reverseItem($item, $staff);
        $cascade = (new PayrollReversalService)->recalculateLater($item->period);

        return response()->json(['reversed' => $result, 'recalculated' => $cascade]);
    }

    public function reversePeriod(Request $request, int $id): JsonResponse
    {
        $period = PayrollPeriod::find($id);
        if (! $period) {
            return response()->json(['error' => 'Payroll period not found'], 404);
        ...
        $result = (new PayrollReversalService)->reversePeriod($period, $staff);
        $cascade = (new PayrollReversalService)->recalculateLater($period->fresh());
